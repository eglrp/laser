...
{ //返回msg_id
	...
	{ //消息超长了
		DMSG((STDOUT, "[%s_%u]FACK SOCKET send error:msg is too long...\r\n", fs->name, fs->id));
		return E_ERROR;
	}
	...
	while (fs->state)
	{
		...
		{
			DMSG((STDOUT, "[%s_%u]FACK SOCKET send timeout...\r\n", fs->name, fs->id));
		...
		DMSG((STDOUT, "[%s_%u]FACK SOCKET send_one_msg done.\r\n", fs->name, fs->id));
		return E_OK;
	}
