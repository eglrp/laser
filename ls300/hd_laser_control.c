...
static e_int32 inter_turn(laser_control_t *lc, float angle);
//state machine:		  ↓￣￣￣￣￣￣￣￣￣￣￣￣￣￣￣￣￣￣￣￣￣￣￣￣↑
...
e_int32 hl_turntable_prepare(laser_control_t *lc)
{
	...
	e_assert(ret, E_ERROR_INVALID_STATUS);
	//先转到开始位置,阻塞的
	ret = inter_turn(lc, lc->start_steps - 200); //冗余处理
	e_assert(ret>0, ret);
...
static e_int32 inter_turn(laser_control_t *lc, float angle)
{
	...
	sprintf(bufSend, MOTO_MSG, (int) ANGLE_TO_STEP(angle), FAST_SPEED);
...
//开始照相
	...
	ret = inter_turn(lc, lc->camera_angle_start); //转至起始角
	e_assert(ret>0, ret);
...
//每隔一定角度拍照一次
	for (start_angle = camera_angle_start; start_angle < camera_angle_end;
			start_angle += TAKEPHOTO_ANGLE)
	{
		ret = inter_turn(lc, TAKEPHOTO_ANGLE);
		e_assert(ret>0, ret);
		Delay(300);
		ret = inter_take_photo(lc);
		e_assert(ret>0, ret);
...
static e_int32 inter_led(laser_control_t *lc, e_uint8 *msg, e_uint32 msg_len)
{
	e_int32 ret;
	e_uint8 buf[MSG_MAX_LEN] =
	{ 0 };
