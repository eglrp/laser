...
static void write_pool_data_routine(void* data)
{
	...
	while (sj->state == STATE_START && sdata.h_angle < sj->start_angle_h
			|| sdata.h_angle > sj->end_angle_h)
	{
		DMSG(
				(STDOUT,"sld_get_measurements sick is not in target position: delay and retry.\r\n"));
		Delay(1000 / sj->speed_v); //等垂直一圈的时间
		sdata.h_angle = hl_turntable_get_angle(&sj->control);
		DMSG(
				(STDOUT,"control trun to %f  @  %f", sj->start_angle_h,sdata.h_angle));
	}

	DMSG(
			(STDOUT,"sld_get_measurements sick is in target position!\r\n start get data.\r\n"));
	...
	while (sj->state == STATE_START)
	{
		...
		if (sdata.h_angle < sj->start_angle_h
				|| sdata.h_angle > sj->end_angle_h)
		{
			DMSG(
					(STDOUT,"sld_get_measurements sick is out of target position again!\r\n stop get data.\r\n"));
			sj->state = STATE_STOP;
			break;
		}
...
static void scan_done(scan_job sj)
{
	...
	sj_save2image(sj->gray_dir, sj->gray, sj->width, sj->height,
			sizeof(e_uint8)); //存储灰度图
}
